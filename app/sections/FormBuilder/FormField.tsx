...
export function FormField({field}: Record<string, any>) {
  ...
  const inputClass = 'theme-input w-full !min-w-0';
  const labelClass = 'theme-body theme-input-label block m-0';
  ...
  return (
    ...
    >
      ...
      {infoMessage && (
        <p className="theme-text-color-faded mt-2 text-xs xs:text-sm">
          {infoMessage}
        </p>
      )}
    </div>
