...
const returnKeyValueIfNotUndefined = (key: string, value?: any) => {
  ...
  (variant: Record<string, any> & {index: number}, index = 0) => {
    try {
      ...
      return {
        ...
        collections: flattenConnection(variant.product?.collections) || [],
      };
    } catch (error) {
  ...
  (line: Record<string, any> & {index?: number}, index = 0) => {
    try {
      ...
      return {
        ...
        collections: flattenConnection(merchandise.product?.collections) || [],
      };
    } catch (error) {
      const message = error instanceof Error ? error.message : error;
