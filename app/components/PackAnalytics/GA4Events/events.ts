...
const logSubscription = ({
  ...
  console.log(
    `GA4Events: subscribed to analytics for \`${packEventName}\`:`,
    data,
  );
};
...
const logError = ({
  ...
  console.error(`GA4Events: error from \`${packEventName}\`: ${message}`);
};
...
const emitEvent = ({
  ...
  try {
    ...
    if (debug)
      console.log(
        `GA4Events: event emitted for \`${emittedEvent.event}\`:`,
        emittedEvent,
      );
  } catch (error) {
    logError({
